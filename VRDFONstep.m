...
if par.Vstate==1  
    ...
    par.Vstate=2; par.ii=1; info.prt=prt; par.comf = 0;
end
...
switch  tune.comBound
 case 0     
    if par.Vstate==4
      if prt>=2, disp('MLS with coordinate directions'); end  
    ...
    if par.Vstate==6
      ...
      if par.ext,
          par.state=0; par.Vstate=5; return
      else
        par.nE=0;
        if par.state==-1, par.state=1; par.Vstate=5; return
        else
            [point,par,step,info] = acceptPoint(point,step,par,tune,info);
            if par.t==par.T && par.state==-1, par.Vstate=7;end
            if par.Vstate~=7, par.Vstate=5; return; end
        end
      end
    end
 case 1
       if par.Vstate==4
        ...
        if par.Vstate==6
              point.fr=f; par.df  = point.fm-point.fr;
              par.ext = (par.df>=tune.gamma*step.alphaE^2 & ...
                                                  par.nE <= tune.E); 
              if par.ext,
                  par.state=0; par.Vstate=5; return;
              else
...
if point.m>=3
   ...
   if par.Vstate==9      
      ...
      par.ext = (par.df>=tune.gamma*step.alphaE^2&par.nE <= tune.E); 
      if par.ext,
          par.state=0; par.Vstate=8; return;
      else
        par.nE=0;
        if par.state==-1, par.state=1; par.Vstate=8; return;
        else
            [point,par,step,info] = acceptPoint...
                                         (point,step,par,tune,info);
            par.Vstate=8; ok = (par.ngood ==0);
            if ok,par.Vstate=10;end
            return
        end
      end
    end
...
if par.Vstate==14
  ...
  par.ext = (par.df>=tune.gamma*step.alphaE^2&par.nE <= tune.E); 
  if par.ext,
      par.state=0; par.Vstate=13; return;
  else
    par.nE=0;
    if par.state==-1, par.state=1; par.Vstate=13; return;
    else
        [point,par,step,info] = acceptPoint(point,step,par,tune,info);
        par.Vstate=13; 
...
if par.Vstate==17 
  ...
  par.ext = (par.df>=tune.gamma*step.alphaE^2&par.nE <= tune.E); 
  if par.ext,
      par.state=0; par.Vstate=15; return
  else
    par.nE=0;
    if par.state==-1, par.state=1; par.Vstate=16; return
    else
        [point,par,step,info] = acceptPoint(point,step,par,tune,info);
        par.Vstate=16; 
        par.totalngood =  par.totalngood+par.ngood;
